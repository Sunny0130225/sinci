...
try {
    ...
    app_log('index.php count failed: ' . $e->getMessage());
    $totalRows = 0;
}
...
try {
    ...
    app_log('index.php query failed: ' . $e->getMessage());
  ...
  <div class="row">
    ...
    <div class="col-md-3">
      <div class="list-group mb-3">
        <a href="index.php" class="list-group-item list-group-item-action <?= $selected_category === null ? 'active' : '' ?>">全部商品</a>
        <?php foreach ($category_options as $main => $subs): ?>
          <a href="index.php?category=<?= urlencode($main) ?>" class="list-group-item list-group-item-action fw-bold <?= $selected_category === $main ? 'active' : '' ?>">
            <?= htmlspecialchars($main, ENT_QUOTES, 'UTF-8') ?>
          </a>
          <?php if ($selected_category === $main || in_array($selected_category, $subs, true)): ?>
            <?php foreach ($subs as $sub): ?>
              <a href="index.php?category=<?= urlencode($sub) ?>" class="list-group-item list-group-item-action ps-4 <?= $selected_category === $sub ? 'active' : '' ?>">
    ...
    <div class="col-md-9">
      ...
      <form class="row g-2 align-items-center mb-3" method="get" action="index.php">
        ...
        <?php if ($q !== '' || $selected_category !== null): ?>
          <div class="col-12">
            <a class="small text-decoration-none" href="index.php">清除篩選</a>
          </div>
        <?php endif; ?>
      </form>
